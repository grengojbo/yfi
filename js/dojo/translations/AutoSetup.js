

if(!dojo._hasResource["translations.AutoSetup"]){ //_hasResource checks added by build. Do not use _hasResource directly in your code.
    dojo._hasResource["tranlations.AutoSetup"] = true;
    dojo.provide("translations.AutoSetup");

(function(){

    translations.AutoSetup.phrases     = {
        'Reload List': { 
            'de_DE' : 'Liste aktualisieren',
            'es_ES' : 'Actualizar Lista',
            'af_ZA' : 'Herlaai lys',
            'fr_FR' : 'Recharger la liste',
            'ms_MY' : 'Senaraikan Semula',
            'nl_NL' : 'Ververs lijst',
            'id_ID' : 'Reload Daftar',
            'th_TH' : 'เรียกรายการมาดูใหม่',
            'pt_BR' : 'Atualizar Lista',
            'da_DK' : 'Genindlæs liste',
            'it_IT' : 'Ricarica lista'
           },
        'Edit Selected': { 
            'de_DE' : 'Auswahl bearbeiten',
            'es_ES' : 'Editar Seleccionado',
            'af_ZA' : 'Redigeer Gekose',
            'fr_FR' : 'Editer la sélection',
            'ms_MY' : 'Ubah Terpilih',
            'nl_NL' : 'Wijzigen',
            'id_ID' : 'Edit yang terpilih',
            'th_TH' : 'แก้ไขที่เลือก',
            'pt_BR' : 'Editar Selecionado',
            'da_DK' : 'Rediger valgte',
            'it_IT' : 'Modifica selezionato'
           },
        'Add': { 
            'de_DE' : 'Hinzuf&uuml;gen',
            'es_ES' : 'Añadir',
            'af_ZA' : 'Nuwe',
            'fr_FR' : 'Ajouter',
            'ms_MY' : 'Tambah',
            'nl_NL' : 'Toevoegen',
            'id_ID' : 'Tambah',
            'th_TH' : 'เพิ่ม',
            'pt_BR' : 'Adicionar',
            'da_DK' : 'Tilføj',
            'it_IT' : 'Aggiungi'
           },
        'Delete Selected': { 
            'de_DE' : 'Auswahl l&ouml;schen',
            'es_ES' : 'Eliminar Seleccionado',
            'af_ZA' : 'Verwyder Gekose',
            'fr_FR' : 'Supprimer la sélection',
            'ms_MY' : 'Padam Terpilih',
            'nl_NL' : 'Verwijderen',
            'id_ID' : 'Hapus yang terpilih',
            'th_TH' : 'ลบที่เลือก',
            'pt_BR' : 'Deletar Selecionado',
            'da_DK' : 'Slet valgte',
            'it_IT' : 'Elimina selezionato'
           },
        'MAC Address': { 
            'de_DE' : 'MAC-Adresse',
            'es_ES' : 'Dirección MAC',
            'af_ZA' : 'MAC-adres',
            'fr_FR' : 'Adresse MAC',
            'ms_MY' : 'Alamat MAC',
            'nl_NL' : 'MAC-adres',
            'id_ID' : 'MAC Address',
            'th_TH' : 'MAC Address',
            'pt_BR' : 'Endereço MAC',
            'da_DK' : 'MAC adresse',
            'it_IT' : 'Indirizzo MAC'
           },
        'IP Address': { 
            'de_DE' : 'IP Adresse',
            'es_ES' : 'Dirección IP',
            'af_ZA' : 'IP-adres',
            'fr_FR' : 'Adresse IP',
            'ms_MY' : 'Alamat IP',
            'nl_NL' : 'IP-adres',
            'id_ID' : 'IP Address',
            'th_TH' : 'IP Address',
            'pt_BR' : 'Endereço IP',
            'da_DK' : 'IP adresse',
            'it_IT' : 'Indirizzo IP'
           },
        'VPN IP Address': { 
            'de_DE' : 'VPN IP Adresse',
            'es_ES' : 'Dirección IP VPN',
            'af_ZA' : 'VPN IP-adres',
            'fr_FR' : 'Adresse IP VPN',
            'ms_MY' : 'Alamat IP VPN',
            'nl_NL' : 'VPN IP-adres',
            'id_ID' : 'VPN IP Address',
            'th_TH' : 'VPN IP Address',
            'pt_BR' : 'Endereço IP da VPN',
            'da_DK' : 'VPN IP adresse',
            'it_IT' : 'Indirizzo IP della VPN'
           },
        'Contact IP': { 
            'de_DE' : 'Kontakt IP',
            'es_ES' : 'IP Contacto',
            'af_ZA' : 'Kontak-IP',
            'fr_FR' : 'Contact IP',
            'ms_MY' : 'IP Hubungan',
            'nl_NL' : 'Verbinding IP',
            'id_ID' : 'IP Contact',
            'th_TH' : 'IP ที่ติดต่อ',
            'pt_BR' : 'IP de Contato',
            'da_DK' : 'Kontakt IP',
            'it_IT' : 'IP Contatto'
           },
        'Last Contact': { 
            'de_DE' : 'Letzter Kontakt',
            'es_ES' : 'Último Contacto',
            'af_ZA' : 'Laaste Kontak',
            'fr_FR' : 'Dernier contact',
            'ms_MY' : 'Hubungan Terakhir',
            'nl_NL' : 'Laatste verbinding',
            'id_ID' : 'Terakhir tersambung',
            'th_TH' : 'ติดต่อครั้งล่าสุด',
            'pt_BR' : 'Último Contato',
            'da_DK' : 'Sidste kontakt',
            'it_IT' : 'Ultimo Contatto'
           },
        'Result count': { 
            'de_DE' : 'Summe',
            'es_ES' : 'Registros',
            'af_ZA' : 'Aantal',
            'fr_FR' : 'Total',
            'ms_MY' : 'Bilangan hasil',
            'nl_NL' : 'Aantal',
            'id_ID' : 'Hasil perhitungan',
            'th_TH' : 'จำนวนผลลัพธ์',
            'pt_BR' : 'Total',
            'da_DK' : 'Antal',
            'it_IT' : 'Risultato'
           },
        'Opening detail for': { 
            'de_DE' : 'Details &ouml;ffnen f&uuml;r',
            'es_ES' : 'Abriendo detalles de',
            'af_ZA' : 'Wys detail vir',
            'fr_FR' : 'Translate me',
            'ms_MY' : 'Membuka perincian untuk',
            'nl_NL' : 'Details openen voor',
            'id_ID' : 'Buka detail untuk',
            'th_TH' : 'เปิดข้อมูลสำหรับ',
            'pt_BR' : 'Abrindo detalhes de',
            'da_DK' : 'Åbner detaljer for',
            'it_IT' : 'Apri dettagli per'
           },
        'No Selection made': { 
            'de_DE' : 'Keine Auswahl getroffen',
            'es_ES' : 'No ha seleccionado nada',
            'af_ZA' : 'Niks gekies nie',
            'fr_FR' : 'Ouverture des détails pour',
            'ms_MY' : 'Tiada pilihan dibuat',
            'nl_NL' : 'Maak eerst een selectie',
            'id_ID' : 'Anda belum memilih',
            'th_TH' : 'ไม่มีการเลือก',
            'pt_BR' : 'Não há nada selecionado',
            'da_DK' : 'Intet valgt',
            'it_IT' : 'Nessuna selezione'
           },
        'New Auto Setup Device': { 
            'de_DE' : 'Neues automatisch konfiguriertes Ger&auml;t',
            'es_ES' : 'TNuevo Dispositivo Autoconfigurable',
            'af_ZA' : 'Nuwe Self-opstel Toestel',
            'fr_FR' : 'Aucun élément',
            'ms_MY' : 'Alat Auto-Setup baru',
            'nl_NL' : 'Nieuw Auto Setupapparaat',
            'id_ID' : 'Auto setup perangkat baru',
            'th_TH' : 'ติดตั้งข้อมูลอุปกรณ์ใหม่แบบอัตโนมัติ',
            'pt_BR' : 'Novo Dispositivo Autoconfigurável',
            'da_DK' : 'Ny enhed med automatisk opsætning',
            'it_IT' : 'Auto configurazione nuovo apparato'
           },
        'Subnet Mask': { 
            'de_DE' : 'Subnet Mask',
            'es_ES' : 'Máscara de Subred',
            'af_ZA' : 'Subnet-masker',
            'fr_FR' : 'Nouvel équipement autoconfiguré',
            'ms_MY' : 'Subnet-Mask',
            'nl_NL' : 'Subnetmasker',
            'id_ID' : 'Subnet Mask',
            'th_TH' : 'Subnet Mask',
            'pt_BR' : 'Máscara de Subrede',
            'da_DK' : 'Subnetmaske',
            'it_IT' : 'Maschera sottorete'
           },
        'Gateway': { 
            'de_DE' : 'Gateway',
            'es_ES' : 'Puerta de Enlace',
            'af_ZA' : 'Gateway',
            'fr_FR' : 'Passerelle',
            'ms_MY' : 'Gateway',
            'nl_NL' : 'Gateway',
            'id_ID' : 'Gateway',
            'th_TH' : 'Gateway',
            'pt_BR' : 'Gateway',
            'da_DK' : 'Gateway',
            'it_IT' : 'Gateway'
           },
        'DNS Server': { 
            'de_DE' : 'DNS Server',
            'es_ES' : 'Servidor DNS',
            'af_ZA' : 'DNS-Bediener',
            'fr_FR' : 'Serveur DNS',
            'ms_MY' : 'DNS-Server',
            'nl_NL' : 'DNS-serve',
            'id_ID' : 'DNS Server',
            'th_TH' : 'DNS Server',
            'pt_BR' : 'Servidor DNS',
            'da_DK' : 'DNS server',
            'it_IT' : 'Server DNS'
           },
        'Save': { 
            'de_DE' : 'Speichern',
            'es_ES' : 'Guardar',
            'af_ZA' : 'Stoor',
            'fr_FR' : 'Sauver',
            'ms_MY' : 'Simpan',
            'nl_NL' : 'Opslaan',
            'id_ID' : 'Simpan',
            'th_TH' : 'บันทึก',
            'pt_BR' : 'Salvar',
            'da_DK' : 'Gem',
            'it_IT' : 'Salva'
           },
        'Created New Auto Setup Device': { 
            'de_DE' : 'Neues Auto-Setup-Ger&auml;t hinzuf&uuml;gen',
            'es_ES' : 'Creado Nuevo Dispositivo Autoconfigurable',
            'af_ZA' : 'Self-opstel Toestel Geskep',
            'fr_FR' : "L'équipement autoconfiguré a été cràe avec succés",
            'ms_MY' : 'Berjaya mencipta Alat Auto-Setup baru',
            'nl_NL' : 'Nieuw Auto Setupapparaat toegevoegd',
            'id_ID' : 'Pembuatan Auto setup perangkat baru',
            'th_TH' : 'ติดตั้งข้อมูลอุปกรณ์ใหม่แบบอัตโนมัติเสร็จแล้ว',
            'pt_BR' : 'Novo Dispositivo Autoconfigurável Criado',
            'it_IT' : 'Configurazione automatica nuovo dispositivo creata'
           },
        'Section' : {
            'de_DE' : 'Abteilung',
            'th_TH' : 'Section',
            'it_IT' : 'Sezione'
           },
        'Note' : {
            'de_DE' : 'Notiz',
            'th_TH' : 'บันทึกย่อ',
            'it_IT' : 'Nota'
           },
        'Date' : {
            'de_DE' : 'Datum',
            'th_TH' : 'วันที่',
            'it_IT' : 'Data'
           },
       'Complete' : {
            'de_DE' : 'Fertig',
            'th_TH' : 'สมบูรณ์',
            'it_IT' : 'Completo'
           },
       'New Note' : {
            'de_DE' : 'Neue Notiz erfassen',
            'th_TH' : 'เพิ่มบันทึกย่อ',
            'it_IT' : 'Nuova nota'
           },
       'Note added' : {
            'de_DE' : 'Notiz hinzugefuegt',
            'th_TH' : 'เพิ่มบันทึกย่อแล้ว',
            'it_IT' : 'Nota aggiunta'
           },
       'Problems adding note' : {
            'de_DE' : 'Probleme beim hnzufuegen der Notiz',
            'th_TH' : 'มีปัญหาในการเพิ่มบันทึกย่อ',
            'it_IT' : 'problema in aggiunta nota'
           },
        'Problems creating Auto Setup Device': { 
            'de_DE' : 'Problem beim hinzuf&uuml;gen Auto-Setup-Ger&auml;t',
            'es_ES' : 'Problemas creando Dispositivo Autoconfigurable',
            'af_ZA' : 'Probleme om self-opstel toestel te skep',
            'fr_FR' : "Des problémes sont survenus lors de la création de l'équipement",
            'ms_MY' : 'Gagal mencipta Alat Auto-Setup baru',
            'nl_NL' : 'Auto Setupapparaat toevoegen mislukt',
            'id_ID' : 'Pembuatan Auto setup perangkat baru bermasalah',
            'th_TH' : 'มีปัญหาในการสร้างการติดตั้งข้อมูลอุปกรณ์แบบอัตโนมัติ',
            'pt_BR' : 'Problemas ao criar novo Dispositivo Autoconfigurável',
            'da_DK' : 'Den automatiske trådløse konfiguration er opdateret',
            'it_IT' : 'problemi durante la creazione automatica di un nuovo dispositivo'
           },
        'never': { 
            'de_DE' : 'niemals',
            'es_ES' : 'nunca',
            'af_ZA' : 'nog nooit',
            'fr_FR' : 'jamais',
            'ms_MY' : 'tidak pernah',
            'nl_NL' : 'nooit',
            'id_ID' : 'Tak pernah',
            'th_TH' : 'ไม่เคย',
            'pt_BR' : 'nunca',
            'da_DK' : 'aldrig',
            'it_IT' : 'mai'
           }
    };

})();//(function(){

}
